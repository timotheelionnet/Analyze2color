function [coloc_sum,coloc_traj,neg_sum,neg_traj,consolidated_trk1,trk1sum,consolidated_trk2,trk2sum] = ...
...
for i=1:ntraj1
    ...
    trk1sum(i,1) = traj1(i);
    trk1sum(i,2) = nanmean(trktmp(:,1));
    trk1sum(i,3) = nanmean(trktmp(:,2));
    trk1sum(i,4) = nanstd(trktmp(:,1));
    trk1sum(i,5) = nanstd(trktmp(:,2));
...
for i=1:ntraj2
    ...
    %store average position, std and start/end frames
    trk2sum(i,1) = traj2(i);
    trk2sum(i,2) = nanmean(trktmp(:,1));
    trk2sum(i,3) = nanmean(trktmp(:,2));
    trk2sum(i,4) = nanstd(trktmp(:,1));
    trk2sum(i,5) = nanstd(trktmp(:,2));
...
coloc_sum = [robust_sum,zeros(size(robust_sum,1),1)];
coloc_traj = robust_traj;

%noncolocalized trajectories
matchidx1 = unique(coloc_sum(:,1)) ;
matchidx1 = matchidx1(matchidx1~=-1);
orphanidx1 = setdiff(trk1sum(:,1) ,matchidx1);

matchidx2 = unique(coloc_sum(:,2)) ;
